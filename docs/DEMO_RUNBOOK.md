...
默认 runner 由 Playwright 直接拥有 `:3210` 的 Next 子进程，并在测试结束时关闭。
`reuseExistingServer: false`：若 `:3210` 已存在监听者，测试立即失败，既不会复用也不会终止它。
整个流程不探测、停止或复用 `:3200`；`E2E_REUSE_SERVER` 不参与 runner 行为。`PROD_URL` 与
`E2E_BASE_URL` 互斥：前者继续走 production smoke，且不会请求开发 fixture endpoint。
