...
| API-AUD-06 | P1（部分关闭） | OpenAPI operation example metadata 审计 | 当前 92 个 operation 中 **46 个没有 request/response example metadata**。`GET/POST /api/access-key`、`GET /api/account/handoffs`、`POST /api/team/invites` 与 `PATCH /api/team/members/{memberId}` 已有可执行 schema 样本；团队写命令的 fixture transition、body idempotency、`401/403/404/409` 语义已在 `ACCOUNT_EXTERNAL_COMMANDS.md` 及 route tests 锁定。TV Show engagement（`GET/POST /api/showcase/{snapshotId}/engagement`）仍无样本，且包含状态机、匿名门与幂等语义，是下一个高风险交接点。 | 为剩余高优先级 operation 至少提供 success、认证/权限失败、幂等 replay/冲突（写命令）样本，并将 examples 接入 OpenAPI `components.examples` 或 operation examples。 |
