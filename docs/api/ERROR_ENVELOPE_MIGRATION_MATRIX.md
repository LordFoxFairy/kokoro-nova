...
| **媒体字节**：`/api/media/{path}` | 文件扩展名决定 `image/svg+xml`、`image/png`、`image/jpeg`、`image/webp`、`video/mp4`、`video/webm`、`audio/wav`、`audio/mpeg`、`text/plain; charset=utf-8` 或 `application/octet-stream`；完整 200 并带缓存与安全 headers | `Response('Forbidden')` / `Response('Not found')`，即 `text/plain; charset=utf-8`，不是 JSON envelope | 200 为 `*/*` 并显式列出 headers；403/404 为 `text/plain; charset=utf-8` string + runtime examples | 已对齐。`<img>`、`<video>`、`<audio>` 以资源 URL 消费 body，不能假设 JSON；把 403/404 强行改 JSON 会改变浏览器资源失败表象、缓存及播放器 fallback。 | 用真实 HTTP 请求断言允许类型、403 traversal、404 缺失文件的 status、content type、CSP、nosniff、cache/range headers；在页面级确认媒体 `onerror`/质量降级不变。若要统一错误，先定义 binary-route 专用错误策略而非让 JSON client 读取它。 |
...
3. **按 transport 分开验收。** `application/json`、`text/event-stream`、媒体字节与 SVG 不是可互换
   的 error channel。SSE 已建流后的生命周期、preview 缺少 error declaration 与 media Range 语义应分别解决，不能用“所有错误都是 JSON”掩盖。
