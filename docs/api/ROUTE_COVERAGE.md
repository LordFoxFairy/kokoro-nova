...
- 资产库的低耦合主切片已由 `src/app/api/assets/route.test.ts` 直接执行：活跃/不可用 lifecycle 列表、资产文件夹创建与计数、元数据移动、软删除、恢复、缺失资产的标准 `404 ErrorResponse`，以及由未绑定生成 artifact 创建 reusable asset 的成功投影与同一 source artifact 的单行 replay。成功响应分别由 `AssetLifecycleListResponseSchema` / `AssetLifecycleViewSchema` 解析，避免只断言 HTTP status。
...
- Jobs/Script V2 主切片已由 `src/app/api/jobs/route.test.ts`、`src/app/api/jobs/[jobId]/route.test.ts`、`src/app/api/script-v2/quotes/route.test.ts`、`src/app/api/script-v2/runs/route.test.ts` 与 `src/app/api/script-v2/runs/[runId]/route.test.ts` 直接执行：生成任务 collection/create、confirmation gate、cancel/retry 的 durable replay；Script V2 quote、queued → running → succeeded 的 polling 状态机、cancel/retry、同 key 同输入 replay 与不同输入 `409`。成功体分别以 jobs/Script V2 的严格 Zod response schema 解析；`400/422` invalid input、`404` missing 与 `409` transition/idempotency conflict 均解析 `LocalErrorEnvelopeSchema`。
...
- 发布主切片已由 `src/app/api/publish/route.test.ts` 与 `src/app/api/publish/[snapshotId]/route.ts` 直接执行：publish 返回严格 summary，public list 仅投影 summary；公开 detail 的 document 是冻结副本，之后的源画布编辑不会渗入；revoke 返回 `revoked` summary，并同时从 public list/detail 隐藏，detail 以标准 `404 ErrorResponse` 统一缺失与下架。
...
- RT-03 Agent 的标准错误 envelope 边界另由 `src/app/api/agent/sessions/__tests__/runtime-contract.test.ts` 直接调用 collection、detail 与 messages handlers：真实消息发送返回严格 `AgentMessagesResponseSchema`（session `seq=2`、`user/assistant` 顺序、未回答 `ask_human`）；空白 `text`、负 `afterSeq` 与空会话分享均返回 schema-valid `400 INVALID_INPUT ErrorResponse`；不存在的可处理 message 返回 schema-valid `404 NOT_FOUND ErrorResponse`。每个错误分支还断言 JSON content type 与稳定的 `req_local_*` requestId / 精确 contract message，防止 handler 将可修正输入泄漏为非标准异常。
...
- 这些仍只是 API-AUD-07 的可重复 route smoke 切片；尚未替代 92 个 operation 的 manifest 驱动 matrix，其余 domain 仍需按 operation 补 success/error wire 断言。
