...
7. 第二次仍冲突时停止自动重放，保留本地恢复项并刷新一次权威画布
8. 以 `role=alert` 展示“已保留本次操作”的恢复提示，并提供“重试保存”按钮
9. 用户重试时，从最新 document 再执行保留的意图函数；成功后才写入一个 undo 帧
```

第二次冲突时，客户端内存中的 `ConflictRecovery` 保存 `label`、重放后的本地 document 快照和
`produce(document)` 意图函数。随后会重新读取服务端的权威 graph，因此远端第二次写入不会被
本地旧快照覆盖；本地操作也不会丢失。恢复项只在重试成功、切换/重载画布时清除，且失败恢复
不会污染 undo/redo 栈。
...
| 连续冲突 | 重放期间远端再写 | 停止自动重试，权威重载、`role=alert` 恢复提示和“重试保存”；双方修改均保留 |
| 两客户端恢复 | A 领先一次，B 重载并重放，然后 undo/redo | 节点/边完整；B 的操作只有成功后才进入正确的 undo/redo 帧 |
