...
- 状态：**交付审计；Linux canonical baseline 尚未引导，视觉 release gate 未启用**
- 审计日期：2026-09-05
...
仓库当前有 **54** 张已跟踪的 `*-darwin.png` 像素基线，全部由 fixture 驱动的 1440×900 状态产生。它们证明 macOS/Darwin 上的局部视觉契约已经存在；它们**不**构成 Ubuntu GitHub Actions 的可比较 release baseline，因为仓库没有任何批准的 `*-linux.png` 文件。
...
| --- | --- | --- |
| Snapshot platform suffix | `git ls-files 'e2e/*snapshots/*.png'` 返回 54 个 `-darwin.png`，没有 `-linux.png`。 | 固定 Linux contract 后只生成/评审同名 `-linux.png`；不可重命名或覆盖 Darwin 基线。 |
...
| Service isolation | 常规 E2E 使用 `:3210/.data-e2e/.next-e2e`；首页 visual spec 需要 `REGRESSION_BASE_URL`。 | visual lane 使用专有 `:3220/.data-visual/.next-visual`，让 `REGRESSION_BASE_URL` 指向同一 Playwright-owned service。 |
| Failure diagnosis | 当前 config 未产出 HTML reporter；失败 trace 会保留。 | 启用 gate 时增加 HTML reporter，并上传 `test-results`（expected/actual/diff、trace）、HTML report、font/browser manifest 与 runner metadata。 |
...
- 54 个 Darwin baseline 的清单、来源 spec 和不跨平台使用的限制均保持可追溯；
...
- Video/Clip 表列出的 P0 状态至少各有一个 approved 1440×900 screenshot contract，不能以单纯 core E2E 替代视觉证据。
