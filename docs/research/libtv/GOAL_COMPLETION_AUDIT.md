...
| 类型、单元、交互和视觉验证 | Typecheck/lint、950 单测、当前列举 28 个隔离核心 E2E（含 Script V2 三阶段/镜头字段/batch gate、编辑 reload、asset/prompt recovery、materialize idempotency、reorder/conflict、Storyboard 与 video-editor handoff）和既有 1440×900 基线均有本地通过证据；TV Show media metadata 截图竞态已由连续 20 次回归覆盖。 | **持续验证中** | 对新补齐的 Clip Editor/Text/API wire contract 扩展同等层级验证；当前绿色测试不能证明尚未实现的能力。 |
