...
- 当前主分支验证基线：提交以 `main` 最新提交为准；本地 `pnpm lint`、`pnpm typecheck`、`pnpm test` 均通过，本轮 142 个 Vitest 文件/1081 个测试；新增的 project collection route smoke、mixed-media video fixture、音轨预览同步、seeded Clip Editor 浏览器 journey、按 project/canvas 隔离的 compose task 与 reset-generation race 已通过隔离验证。GitHub Actions 仍以最新 run 的最终状态为准。
...
| 完整 Video 创作与剪辑体验 | 视频模型目录、生成 job、确认门、媒体回退、Storyboard、compose lifecycle 和导出 mock 已有契约；`clip-editor-core` 覆盖裁切、分割、变速、转场、字幕、独立音轨、音量/静音、reload、失败/取消，`video-media-interaction-core` 覆盖 HTML5 drag/drop、preview 播放、reload 与本地导出不污染 timeline；`video-succeeded` fixture 现在提供双视频、独立音轨、转场、字幕、稳定 playhead/zoom，并由 Clip Editor seeded journey 回读验证；seeded mixed-media、local export success/failed 均有独立 Darwin 像素基线。 | **主路径已证实** | 继续以官网只读观察校准可见交互，并建立 Linux canonical 视觉基线；当前本地 mock 通过不等同于官网完成态。 |
| 所有 API、任务、素材、数据均可重复的本地 mock/fixture | 55 path / 92 operation 清单、scenario/reset、fixture media、jobs、presence、identity 和 account handoff 均在仓内；compose task 已记录 project/canvas scope，scenario reset 会清理旧 task/成片并使旧 worker 失效；媒体不再宣称未实现的 byte range。 | **主路径已证实** | 为长任务、资产和跨 workspace 状态补 manifest 驱动 route smoke，防止 future adapter 靠 UI 分支猜测。 |
...
| 类型、单元、交互和视觉验证 | Typecheck/lint、142 个 Vitest 文件/1081 个测试、当前核心 suite 已列入 Project collection、Script V2、Storyboard/video-editor handoff、Clip Editor deterministic journey 与 seeded mixed-media journey，另有 1440×900 Darwin 基线覆盖 loaded/selected/seeded/export success/export failed 状态；首页动画 banner 的 visual contract 已固定到提交的首帧 fixture。 | **持续验证中** | 建立 Linux canonical visual baseline，并继续补官网只读证据；当前绿色测试不能证明尚未实现的能力。 |
