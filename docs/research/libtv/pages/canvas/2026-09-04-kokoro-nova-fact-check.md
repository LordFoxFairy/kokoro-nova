...
- [ ] **P0-CANVAS-01：History 查询态补齐。** 在 `HistoryPanel` 增加 `scope = canvas | image | video | audio`、评级筛选、时间倒序、批量操作入口；空集合仍显示全部控件。查询条件只存在于 panel state/API query，不写入 `WorkflowDocument`。增加 `library-panels` 单测和一条桌面 E2E。
