...
合成器的本地任务在 `queued → rendering → succeeded | failed | cancelled` 间转换，并为每次
提交保留一个可轮询的任务标识。刷新会重新读取该任务的持久化本地状态；失败态可原任务重试；
取消仅终止合成任务，不改写时间线，也不写入视频产物。成功任务恰好创建一个画布可用的视频
产物。这是前端 fixture 生命周期证据，不对应官网后台任务接口或任何远端模型调用。
...
#### 2026-09-04 登录态 Script V2 抽屉复核

来源：用户已登录的官网画布，只读观察 `打开脚本节点 →` 后的 Script V2 状态；未提交生成、下载或任何云端修改。

- 入口展开为**底部抽屉式故事板**，而不是离开当前画布的独立路由；顶栏依次有`重新生成`、`批量生成分镜`、`批量生视频`、`下载`。在当前 `1` 个镜头、`0/1` 资产已生成、`0/1` 提示词已合成的前置不完整状态，后三个批量/下载动作均 disabled。
- 三阶段进度精确显示：`确认镜头 1个镜头已就绪`、`准备资产 0/1 已生成、还差1个`、`合成提示词 0/1 已合成`；进度提示为`1/3 完成后可批量生视频`。
- 第一阶段表头顺序固定为：镜号、时长、画面描述、景别、光影氛围、对白·旁白、音效、运镜、最终提示词、操作。镜头行支持拖拽重排；时长（当前 `5s`）、画面描述、光影/对白/音效/运镜可编辑，景别为选择字段，最终提示词通过`点击查看提示词`进入详情，行尾是动作菜单。
- 阶段一底栏保留`添加镜头`与`下一步：准备资产`。本地实现据此改为底部 drawer，并将先前错误的“从确认镜头直接一键合成提示词”主动作改为明确进入资产准备阶段；提示词批量合成仍位于第三阶段。
...
项目管理回归现在使用独立的 `e2e/project-lifecycle.spec.ts`：从
`authenticated-empty` fixture 新建并重命名文件夹，创建项目，选择本地示例封面，创建副本，将副本移动到文件夹，
再刷新并确认文件夹名称、原项目封面、副本封面和移动结果仍然存在。每个测试结束后
通过 `/api/dev/scenario` 恢复空工作区，不留下测试生成物。
