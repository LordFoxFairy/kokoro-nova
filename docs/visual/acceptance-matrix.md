...
已有隔离浏览器证据覆盖桌面基线、项目/画布加载错误、空态、有内容态，以及视频任务的过期/有效等待确认、排队、运行、成功、失败、取消和合规阻断。首页和项目页现在也有独立的 `1440×900` Playwright 像素快照守门，基线固定在可重复的 `authenticated-populated` fixture；响应式跟进中记录的两个已修复问题仍成立：窄屏主工具栏的独立 `translate` 已清除，活动倒计时胶囊已禁止收缩换行。
