...
test('Minimax inserts exact pause and paralinguistic tokens at the current caret', async ({ page, request }) => {
  ...
  const pauseTrigger = editor.getByRole('button', { name: '<#> 停顿' })
  await pauseTrigger.click()
  const pauseMenu = page.getByTestId('audio-pause-menu')
  await expect(pauseTrigger).toHaveAttribute('aria-expanded', 'true')
  ...
  const cueTrigger = editor.getByRole('button', { name: '() 语气词' })
  await cueTrigger.click()
  const cueMenu = page.getByTestId('audio-cue-menu')
  await expect(cueTrigger).toHaveAttribute('aria-expanded', 'true')
