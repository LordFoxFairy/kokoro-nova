import { LEDGER_MAX_LIMIT, LedgerViewProjectionSchema } from '@/contracts/ledger'
...
function parseLimit(raw: string | null): number | undefined {
  ...
  return Math.min(value, LEDGER_MAX_LIMIT)
}
...
export async function GET(request: Request) {
  return handle(async () => {
    ...
    return LedgerViewProjectionSchema.parse(projectLedger(state, DEFAULT_SPACE_ID, limit))
  })
}
