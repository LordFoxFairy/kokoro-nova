...
export interface AudioModelCatalogProps {
...
/** Six-item catalogue frozen from the current Audio node selector. */
export function AudioModelCatalog({ selectedId, onSelect, onClose, className }: AudioModelCatalogProps) {
  const [query, setQuery] = useState('')
  const [activeIndex, setActiveIndex] = useState(0)
  const inputRef = useRef<HTMLInputElement>(null)
  const models = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase('zh-CN')
    if (!needle) return modelsFor('audio')
    return modelsFor('audio').filter((model) =>
  ...
  return (
    <div
      ...
      aria-modal="false"
      aria-label="音频模型目录"
      data-testid="audio-model-catalog"
      onKeyDown={(event) => {
        if (event.key === 'Escape') {
    ...
    >
      ...
      <div className="thin-scrollbar min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
        {models.map((model, index) => {
          ...
          return (
            <button
              key={model.id}
              type="button"
              data-testid={`audio-model-option-${model.id}`}
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[13px] font-medium text-ink-900">{model.label}</span>
        ...
        {models.length === 0 && (
          <div className="flex h-32 items-center justify-center text-[12px] text-ink-400">没有匹配的音频模型</div>
        )}
      </div>
    </div>
  )
...
export function AudioModelMark({ iconKey }: { iconKey?: string }) {
  return (
    <span
      data-model-icon={iconKey ?? 'audio'}
      aria-hidden="true"
    ...
    >
      {iconKey === 'mureka' ? (
        ...
        <span className="flex h-3 items-center gap-[2px]">
          <i className="h-1.5 w-[2px] rounded-full bg-current" />
          <i className="h-3 w-[2px] rounded-full bg-current" />
          <i className="h-2 w-[2px] rounded-full bg-current" />
          <i className="h-2.5 w-[2px] rounded-full bg-current" />
        </span>
      )}
    </span>
  )
