...
import { creationContextClient } from "@/api/creation-context";
...
import {
  CREATION_CONTEXT_STORAGE_KEY,
  creationContextSummary,
  emptyCreationContext,
  normalizeCreationContext,
  readCreationContextStorage,
  withCreationAttachment,
  withCreationModel,
  withCreationReference,
  withCreationSkill,
  withoutCreationAttachment,
  withoutCreationReference,
} from "@/domain/creation-context";
...
export type HomeAgentContextKind = "asset" | "reference" | "skill";
...
export type HomeAgentRequest = {
  text: string;
  /** Immutable, versioned snapshot accepted by the local Agent request boundary. */
...
export function buildHomeAgentBrief(request: HomeAgentRequest): string {
  return [
    request.text.trim(),
    ...creationContextSummary(request.creationContext),
  ].join("\n");
}
...
export function nextHomeComposerEscapeState(input: {
  ...
  if (input.activePopover)
    return { expanded: input.expanded, activePopover: null, handled: true };
  if (input.expanded)
    return { expanded: false, activePopover: null, handled: true };
...
export function HomeAgentComposer({
  ...
  const [assetLibraryMode, setAssetLibraryMode] = useState<
    "attachment" | "reference" | null
  >(null);
  const [assetSearch, setAssetSearch] = useState("");
  const [assetCategory, setAssetCategory] =
    useState<(typeof ASSET_CATEGORIES)[number]>("全部");
  ...
  const [selectedSkill, setSelectedSkill] = useState<CreationSkill | null>(
    null,
  );
  const [selectedModel, setSelectedModel] = useState<CreationModel | null>(
    null,
  );
  ...
  const creationContext = useMemo(
    () =>
      normalizeCreationContext({
        version: emptyCreationContext().version,
        attachments,
        model: selectedModel,
        skill: selectedSkill,
        references,
    ...
    [attachments, generationMode, references, selectedModel, selectedSkill],
  );
  const context = useMemo<HomeAgentContext[]>(
    () => [
    ...
    [attachments, references, selectedSkill],
  );
  const expanded =
    focused ||
    draft.trim().length > 0 ||
  ...
  const valid = draft.trim().length > 0 && !submitting && !contextSubmitting;
  ...
  const loadAssets = useCallback(async () => {
    ...
    try {
      const response = await client.raw.get<{ assets: Asset[] }>(
        "/api/assets?namespace=personal",
      );
      ...
      setAssetState({
        status: "error",
        items: [],
        error: errorMessage(reason, "个人资产库加载失败"),
      });
    }
  }, []);

  const loadModels = useCallback(
    async (media: "image" | "video", force = false) => {
      ...
      setModelStates((states) => ({
        ...states,
        [media]: { ...states[media], status: "loading", error: null },
      }));
      try {
        const response = await client.models.list({ media });
        setModelStates((states) => ({
          ...states,
          [media]: { status: "ready", items: response.items, error: null },
        }));
      } catch (reason) {
        setModelStates((states) => ({
          ...states,
          [media]: {
            status: "error",
            items: [],
            error: errorMessage(reason, "模型目录加载失败"),
          },
        }));
      }
    },
  ...
  const loadSkills = useCallback(
    async (collection: "全部" | "收藏" | "我的", query: string) => {
      ...
      try {
        ...
        setSkillState({
          status: "error",
          items: [],
          error: errorMessage(reason, "Skill 目录加载失败"),
        });
      }
    },
    [],
  ...
  useEffect(() => {
    if (
      ...
      return;
    void loadAssets();
  }, [assetLibraryMode, assetState.status, loadAssets]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(
      () => void loadSkills(skillCollection, skillSearch),
      160,
    );
  ...
  // Browser storage restores a page refresh; the PUT mirror is the local Agent
  // gateway seam and lets a future session bind exactly this normalized snapshot.
  useEffect(() => {
    const restored = readCreationContextStorage(
      window.localStorage.getItem(CREATION_CONTEXT_STORAGE_KEY),
    );
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      CREATION_CONTEXT_STORAGE_KEY,
      JSON.stringify(creationContext),
    );
    void creationContextClient.save(creationContext).catch(() => {
      // The browser copy remains the recovery source when a disposable mock is restarting.
    });
  }, [contextHydrated, creationContext]);

  useEffect(() => {
    const onPointerDown = (event: PointerEvent) => {
      if (assetLibraryMode) return;
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key !== "Escape" || assetLibraryMode) return;
  ...
  }, [activePopover, assetLibraryMode, context.length, draft, expanded]);
  ...
  const addAttachment = (next: CreationAttachment) => {
    setAttachments(
      (current) =>
        withCreationAttachment(
          { ...creationContext, attachments: current },
          next,
        ).attachments,
    );
    setFocused(true);
  ...
  const addReference = (next: CreationReference) => {
    setReferences(
      (current) =>
        withCreationReference({ ...creationContext, references: current }, next)
          .references,
    );
    setFocused(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submitted = await creationContextClient.submit(
        prompt,
        creationContext,
      );
      onSubmit({
        text: prompt,
        creationContext: submitted.request.context,
        creationRequestId: submitted.request.id,
      });
    } catch (reason) {
      setContextError(errorMessage(reason, "创作上下文保存失败，请重试"));
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assetState.items.filter((asset) => {
      const categoryMatch =
        assetCategory === "全部" || asset.tags.includes(assetCategory);
      const searchMatch =
        !query || asset.name.toLocaleLowerCase("zh-CN").includes(query);
    ...
    >
      <div className="relative w-full max-w-[720px]">
        {expanded && (
          ...
          >
            ...
            <span>
              {modeLabel(generationMode)} ·{" "}
              {generationMode === "auto" ? "可能消耗积分" : "生成前询问"}
            </span>
            {selectedModel && (
        ...
        <div
          className={cn(
            ...
            expanded
              ? "min-h-[128px] border border-white/[0.08]"
              : "h-12 border border-transparent",
                  ...
                  onRemove={() => {
                    if (item.kind === "skill")
                      ...
                      setReferences(
                        (current) =>
                          withoutCreationReference(
                            { ...creationContext, references: current },
                            item.id,
                          ).references,
                      );
                    else
          ...
          {expanded && (
            <div className="flex items-center justify-between px-2 pb-2">
              ...
              <div className="flex items-center gap-0.5">
                ...
                >
                  {selectedModel ? (
                    modelIcon({ media: modelMedia }, 15)
                  ) : (
                    <IconImage size={16} />
                  )}
                  <span className="max-w-[116px] truncate text-[11px]">
              ...
              <button
                ...
                className={cn(
                  ...
                  valid
                    ? "bg-white text-[#181818] hover:bg-white/88"
                    : "cursor-not-allowed bg-white/[0.09] text-white/36",
                )}
              >
        ...
        {activePopover === "attachments" && (
          <ComposerPopover
            testId="home-attachment-menu"
            title="添加素材"
            className="left-0 w-[210px]"
          >
            <PopoverAction
            ...
            <PopoverAction
              ...
              label="素材库添加"
              description="从个人资产库添加附件"
              onClick={() => {
                setActivePopover(null);
                setAssetLibraryMode("attachment");
              }}
            ...
            <PopoverAction
              ...
              label="添加参考"
              description="从个人资产库引用为生成参考"
              onClick={() => {
                setActivePopover(null);
                setAssetLibraryMode("reference");
              }}
              testId="home-reference-library"
            />
          </ComposerPopover>
        )}
        ...
        {activePopover === "model" && (
          <ComposerPopover
            testId="home-model-menu"
            title="选择模型"
            className="left-0 w-[330px]"
          >
            {selectedModel && (
              <button
                type="button"
                data-testid="home-model-clear"
                onClick={() => {
                  setSelectedModel(
                    (current) =>
                      withCreationModel(creationContext, current ? null : null)
                        .model,
                  );
                  setActivePopover(null);
                }}
                className="mb-2 rounded-md px-1.5 py-1 text-[11px] text-white/48 hover:bg-white/[0.06] hover:text-white"
              >
                取消已选模型
              </button>
            )}
            <div
              className="grid grid-cols-2 gap-1 rounded-lg bg-white/[0.05] p-1"
              role="tablist"
              aria-label="模型类型"
            >
              {MEDIA_TABS.map((tab) => (
            ...
            <ModelList
              ...
              onSelect={(model) => {
                setSelectedModel({
                  id: model.id,
                  label: model.label,
                  media: model.media as "image" | "video",
        ...
        {activePopover === "skill" && (
          <ComposerPopover
            testId="home-skill-menu"
            title="Skill 上下文"
            className="left-0 w-[390px]"
          >
                ...
                {SKILL_COLLECTIONS.map((collection) => (
                  <button
                    ...
                    className={cn(
                      ...
                      skillCollection === collection.value
            ...
            <Link
              href="/skills"
              className="mt-2 flex items-center justify-center border-t border-white/[0.07] pt-2 text-[11px] text-[#60c9ef] hover:text-[#8cdbf5]"
            >
              查看全部 Skill
            </Link>
          </ComposerPopover>
        ...
        {activePopover === "mode" && (
          <ComposerPopover
            testId="home-mode-menu"
            title="生成模式"
            className="right-0 w-[230px]"
          >
      ...
      <div className="mt-3 flex max-w-full items-center justify-center gap-2">
          ...
          return (
                    ...
                    withCreationSkill(
                      ...
                      selected
                        ...
                        : {
                            id: skill.id,
                            label: skill.name,
                            version: "featured-local-2026-09-04.1",
                          },
                    ).skill,
                );
                setFocused(true);
            ...
            >
              ...
              <span className="truncate">
                <span className="mr-1 text-white/36">使用 Skill</span>
                {skill.name}
              </span>
            </button>
          );
        ...
        <Link
          href="/skills"
          aria-label="全部 Skill"
          className="flex h-8 shrink-0 items-center gap-1 rounded-full bg-white/[0.045] px-3 text-[12px] text-white/65 transition-colors hover:bg-white/[0.08] hover:text-white"
        >
      ...
      {contextError && (
        <p
          role="alert"
          data-testid="home-creation-context-error"
          className="mt-2 text-center text-[11px] text-red-200"
        >
          {contextError}
        </p>
      )}
...
function PopoverAction({
  ...
  return (
    ...
    >
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/[0.06] text-white/72">
        {icon}
      </span>
...
function ContextPill({
  ...
  item: HomeAgentContext;
  onRemove: () => void;
}) {
  return (
    <span
      data-testid="home-context-chip"
      className="inline-flex max-w-[220px] items-center gap-1.5 rounded-full bg-[#60c9ef]/12 px-2 py-1 text-[10px] text-[#9be2fa]"
    >
      {item.thumbnailUrl ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img
      ...
      <button
        ...
        onClick={onRemove}
        className="rounded-full text-[#9be2fa]/60 hover:text-white"
      >
        <IconClose size={10} />
      </button>
    </span>
...
function ModelList({
  ...
  if (state.status === "loading") {
    return (
      <div
        className="flex items-center justify-center gap-2 py-8 text-[11px] text-white/42"
        data-testid="home-model-loading"
      >
        <Spinner size={14} /> 加载模型目录
      </div>
    );
  }
  if (state.status === "error") {
    return (
      <InlineLoadError
        message={state.error ?? "模型目录加载失败"}
        onRetry={onRetry}
        testId="home-model-error"
        retryTestId="home-model-retry"
      />
    );
  }
  if (state.items.length === 0) {
    return (
      <InlineEmpty
        title={`${media === "image" ? "图片" : "视频"}模型暂时为空`}
        description="请稍后刷新本地目录"
        testId="home-model-empty"
      />
    );
  }
  return (
    <div
      className="thin-scrollbar mt-1 max-h-[238px] space-y-1 overflow-y-auto pr-1"
      data-testid="home-model-list"
    >
      {state.items.map((model) => (
        ...
        >
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/[0.07] text-white/75">
            {modelIcon({ media }, 16)}
          </span>
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-1.5 text-[12px] text-white/86">
              <span className="truncate">{model.label}</span>
              {model.membershipTier === "vip" && (
                <span className="shrink-0 rounded bg-amber-300/15 px-1 text-[9px] text-amber-200">
                  VIP
                </span>
              )}
            </span>
          ...
          {selectedId === model.id && (
            <IconCheck size={14} className="shrink-0 text-[#60c9ef]" />
          )}
        </button>
      ))}
    </div>
...
function SkillList({
  ...
  selectedId: string | null;
  onRetry: () => void;
  onSelect: (skill: Pick<SkillCardContract, "id" | "name" | "summary" | "version">) => void;
}) {
  if (state.status === "loading") {
    return (
      <div
        className="flex items-center justify-center gap-2 py-10 text-[11px] text-white/42"
        data-testid="home-skill-loading"
      >
        <Spinner size={14} /> 加载 Skill 目录
      </div>
    );
  }
  if (state.status === "error") {
    return (
      <InlineLoadError
        message={state.error ?? "Skill 目录加载失败"}
        onRetry={onRetry}
        testId="home-skill-error"
        retryTestId="home-skill-retry"
      />
    );
  }
  if (state.items.length === 0) {
    return (
      <InlineEmpty
        title="暂无 Skill"
        description="可以从全部 Skill 中创建一个能力包"
        testId="home-skill-empty"
      />
    );
  }
  return (
    <div
      className="thin-scrollbar mt-2 max-h-[240px] space-y-1 overflow-y-auto pr-1"
      data-testid="home-skill-list"
    >
      {state.items.map((skill) => (
        ...
        >
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white/[0.07] text-[#9be2fa]">
            <IconSkill size={14} />
          </span>
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-1.5 text-[12px] text-white/86">
              <span className="truncate">{skill.name}</span>
              <span className="shrink-0 rounded bg-white/[0.06] px-1 text-[9px] text-white/32">
                /{skill.id.replace(/^skill-/, "")}
              </span>
            </span>
            <span className="mt-0.5 block line-clamp-2 text-[10px] leading-relaxed text-white/35">
              {skill.summary}
            </span>
          </span>
          {selectedId === skill.id && (
            <IconCheck size={14} className="mt-1 shrink-0 text-[#60c9ef]" />
          )}
        </button>
      ))}
    </div>
    ...
    >
      <span
        className={cn(
          ...
          selected
            ? "border-[#60c9ef] bg-[#60c9ef] text-[#17242a]"
...
function InlineLoadError({
  ...
  return (
    <div
      data-testid={testId}
      role="alert"
      className="my-2 flex items-start gap-2 rounded-xl border border-red-300/15 bg-red-300/[0.06] p-2.5 text-[10px] text-red-200/78"
    >
      ...
      <button
        ...
        onClick={onRetry}
        className="shrink-0 rounded px-1.5 py-0.5 font-medium text-red-100 hover:bg-red-200/10"
      >
        重试
      </button>
    </div>
...
function AssetLibraryDialog({
  ...
  mode: "attachment" | "reference" | null;
  open: boolean;
  state: AssetState;
  search: string;
  ...
  return (
    <Dialog
      ...
      width={880}
      hideHeader
      testId="home-asset-library-dialog"
    >
      <div className="flex min-h-[520px] flex-col text-white">
        <header className="flex items-center justify-between border-b border-white/[0.08] px-1 pb-3">
          <div>
            <h2 className="text-[15px] font-semibold text-white/88">
              {mode === "reference" ? "添加参考" : "资产管理"}
            </h2>
          ...
          <button
            ...
            aria-label="关闭"
            onClick={onClose}
            className="rounded-lg p-1.5 text-white/45 hover:bg-white/[0.07] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#60c9ef]"
          >
            <IconClose size={17} />
          </button>
        </header>
        <div className="flex items-center gap-2 py-3">
          ...
          <button
            type="button"
            data-testid="home-asset-upload"
            onClick={onUpload}
            className="flex h-9 shrink-0 items-center gap-1.5 rounded-lg bg-white px-3 text-[12px] font-medium text-[#181818] hover:bg-white/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#60c9ef]"
          >
            <IconUpload size={14} /> 上传资产
          </button>
        </div>
        <div
          className="flex flex-wrap items-center gap-1 border-b border-white/[0.08] pb-3"
          role="tablist"
          aria-label="资产分类"
        >
          {ASSET_CATEGORIES.map((item) => (
            <button
              ...
              data-testid={`home-asset-category-${item}`}
        ...
        <div className="thin-scrollbar flex-1 overflow-y-auto">
          {state.status === "loading" ? (
            <div
              data-testid="home-asset-loading"
              className="flex min-h-[330px] items-center justify-center gap-2 text-[12px] text-white/42"
            >
              <Spinner size={16} /> 加载个人资产库
            </div>
          ) : state.status === "error" ? (
            <div className="flex min-h-[330px] items-center justify-center">
              <InlineLoadError
                message={state.error ?? "个人资产库加载失败"}
                onRetry={onRetry}
                testId="home-asset-error"
                retryTestId="home-asset-retry"
              />
            </div>
          ) : filteredAssets.length === 0 ? (
            <div
              data-testid="home-asset-empty"
              className="flex min-h-[330px] flex-col items-center justify-center gap-2 text-center"
            >
              <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/[0.06] text-white/24">
                <IconAssetLibrary size={27} />
              </span>
              ...
              <button
                type="button"
                onClick={onUpload}
                className="mt-1 rounded-lg bg-white px-3 py-1.5 text-[11px] font-medium text-[#181818] hover:bg-white/85"
              >
                上传资产
              </button>
            </div>
            ...
            >
              {filteredAssets.map((asset) => (
                <button
                  key={asset.id}
                  type="button"
                  data-testid="home-asset-option"
                  onClick={() => onSelect(asset)}
                  className="group overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.035] text-left transition-colors hover:border-[#60c9ef]/55 hover:bg-white/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#60c9ef]"
                >
                  ...
                  <span className="block truncate px-2.5 py-2 text-[11px] text-white/75">
                    {asset.name}
                  </span>
                </button>
              ))}
