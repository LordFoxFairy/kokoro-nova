...
import { listShowcasePage } from '@/api/showcase'
...
/** Historical adapter retained for frozen snapshot consumers. */
...
/** Mirrors the observed recommendation fallback for consumers outside the paged gallery. */
export function filterShowcaseEntries(
  ...
  const matches = categoryEntries.filter((entry) => `${entry.title} ${entry.summary} ${entry.author}`.toLocaleLowerCase('zh-CN').includes(needle))
  return matches.length > 0 ? { entries: matches, fallback: false } : { entries: categoryEntries, fallback: categoryEntries.length > 0 }
}
...
export function getShowcaseRequestState({ loading, hasData, error }: { loading: boolean; hasData: boolean; error: string | null }): ShowcaseRequestState {
...
export function ShowcaseGallery() {
  const [response, setResponse] = useState<ShowcaseListResponse | null>(null)
  ...
  const requestState = getShowcaseRequestState({ loading, hasData: entries.length > 0, error })
  const feedback = useMemo(() => getShowcaseSearchFeedback({
    category,
    query,
  ...
  return (
    <div className="min-h-screen bg-[#111] text-white" data-testid="showcase-gallery" aria-busy={loading}>
      <aside className="flex h-12 items-center justify-center overflow-hidden border-b border-white/[0.05] bg-[#d7f3fb] px-4 text-[12px] text-[#17232a]">
        <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-[#ec168c] px-3 py-1 font-medium text-white"><span aria-hidden="true">⏱</span> 活动剩余 5 天 23 时</span>
        <span className="ml-3 truncate font-medium">Agent 全面上线，专业 Skills 助力创作</span>
        <Link href="/account" className="ml-3 shrink-0 rounded-full border border-[#17232a] px-3 py-1 font-medium transition-colors hover:bg-[#17232a] hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">限时抢购</Link>
      </aside>
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.07] px-4 py-4 sm:px-8">
        <Link href="/" className="flex items-center gap-2 text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"><LibTvLogo className="h-6 w-[88px]" /><span className="sr-only">Kokoro Nova · LibTV</span></Link>
        <nav aria-label="公开探索导航" className="flex items-center gap-2">
          <Link href="/skills" className="rounded-lg border border-white/[0.08] px-3 py-2 text-[12px] text-white/70 transition-colors hover:bg-white/[0.06] hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">Skills</Link>
          <Link href="/account" className="rounded-lg border border-white/[0.08] px-3 py-2 text-[12px] text-white/70 transition-colors hover:bg-white/[0.06] hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">账户</Link>
        ...
        <div className="mx-auto max-w-[1500px]">
          <div className="flex flex-wrap items-baseline justify-between gap-2 pt-7">
            ...
            <div id="showcase-status" className="text-[12px] text-white/45" data-testid="showcase-status" role="status" aria-live="polite">
              {loadingKind === 'initial' ? '正在加载公开作品…' : loadingKind === 'search' ? '正在搜索公开作品…' : loadingKind === 'more' ? '正在加载更多作品…' : ''}
            </div>
          </div>
          <div className="mt-6 flex flex-col gap-3 border-b border-white/[0.07] pb-3 lg:flex-row lg:items-center">
            {!query && <div className="no-scrollbar flex min-w-0 flex-1 gap-1 overflow-x-auto" role="group" aria-label="TV Show 分类">
              {SHOWCASE_CATEGORIES.map((item) => <button key={item} type="button" data-testid={`showcase-category-${item}`} data-selected={category === item ? 'true' : undefined} aria-pressed={category === item} onClick={() => setCategory(item)} className={cn('shrink-0 rounded-lg border px-3 py-1.5 text-[12px] transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent', category === item ? 'border-white/[0.16] bg-white/[0.12] text-white' : 'border-white/[0.07] text-white/48 hover:bg-white/[0.06] hover:text-white/80')}>{item}</button>)}
            </div>}
            <form role="search" aria-label="搜索公开作品" onSubmit={(event) => { event.preventDefault(); setQuery(draftQuery.trim()) }} className="flex h-9 w-full shrink-0 items-center gap-2 rounded-full border border-white/[0.1] px-3 text-white/35 transition-colors focus-within:border-white/25 lg:ml-auto lg:w-64">
              <label className="sr-only" htmlFor="showcase-search">搜索 TV Show</label>
              <input id="showcase-search" data-testid="showcase-search" inputMode="search" value={draftQuery} onChange={(event) => setDraftQuery(event.target.value)} placeholder="请输入搜索内容" aria-label="搜索 TV Show" className="min-w-0 flex-1 bg-transparent text-[12px] text-white/85 outline-none placeholder:text-white/28 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent" />
              {(draftQuery || query) && <button type="button" data-testid="showcase-clear-search" aria-label="清除搜索" onMouseDown={(event) => event.preventDefault()} onClick={() => { setDraftQuery(''); setQuery('') }} className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-white/45 transition-colors hover:bg-white/[0.1] hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"><IconClose size={12} /></button>}
              <button type="submit" data-testid="showcase-search-submit" aria-label="提交搜索" className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-white/45 transition-colors hover:bg-white/[0.1] hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"><IconSearch size={14} /></button>
            </form>
          </div>
          <p data-testid="showcase-search-feedback" aria-live="polite" className="mt-3 min-h-4 text-[11px] text-white/35">{feedback}</p>
          {loadingKind === 'search' && <div data-testid="showcase-search-loading" className="mt-3 flex items-center gap-2 text-[11px] text-white/45" role="status"><Spinner size={14} /> 正在搜索公开作品…</div>}
...
function ShowcaseCard({ entry }: { entry: ShowcaseEntryProjection }) {
  return <Link href={`/showcase/${entry.id}`} data-testid={`showcase-card-${entry.id}`} className="group flex min-w-0 flex-col rounded-xl focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent">
    <span className="relative flex aspect-video w-full items-center justify-center overflow-hidden rounded-xl bg-[#222] ring-1 ring-white/[0.07] transition-shadow group-hover:shadow-[0_0_0_1px_rgba(255,255,255,.18),0_12px_28px_rgba(0,0,0,.28)]">{entry.coverUrl ? <img src={entry.coverUrl} alt={entry.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.025]" /> : <IconImage size={34} className="text-white/25" aria-hidden="true" />}<span className="absolute bottom-2 right-2 rounded-full bg-black/65 px-2 py-1 text-[10px] text-white/80 backdrop-blur">查看制作过程 <IconChevronRight size={11} className="ml-0.5 inline" /></span></span>
    <span className="mt-2.5 block truncate text-[14px] font-medium text-white/90">{entry.title}</span>{entry.summary && <span className="mt-0.5 line-clamp-2 text-[12px] leading-relaxed text-white/42">{entry.summary}</span>}<span className="mt-1.5 flex items-center justify-between gap-2 text-[12px] text-white/40"><span className="truncate">{entry.author}{entry.authorTier && <span className="ml-1.5 text-[#d3aa68]">{entry.authorTier}</span>}</span><span className="shrink-0">♡ {entry.likeCount}</span></span><span className="mt-1 flex items-center gap-2 text-[11px] text-white/30">{new Date(entry.publishedAt).toLocaleDateString('zh-CN')}<span className="flex items-center gap-1"><IconLayers size={12} />{entry.nodeCount} 个节点</span></span>
  </Link>
}
