...
export function getSkillDetailRequestState({
  loading,
  hasSkill,
  error,
}: {
...
export function SkillDetail({ skillId }: { skillId: string }) {
  ...
  return (
    <div className="min-h-screen bg-surface" data-testid="skill-detail" aria-busy={loading}>
      <header className="flex items-center justify-between px-8 py-5">
        <Link
          href="/skills"
          className="flex items-center gap-1 text-[13px] text-ink-600 transition-colors hover:text-ink-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
        >
      ...
      {loading ? (
        <div className="flex justify-center py-24 text-ink-600" role="status" aria-label="正在加载 Skill 详情">
          <Spinner size={22} />
        </div>
      ) : !skill ? (
        <div className="px-8 pb-20">
          <EmptyState
            ...
            title={error ? 'Skill 暂时加载失败' : '这个 Skill 不存在'}
        ...
        <main className="mx-auto max-w-3xl px-8 pb-20" aria-labelledby="skill-detail-title">
          ...
          <h1 id="skill-detail-title" className="mt-5 text-[22px] font-semibold tracking-tight text-ink-900">{skill.name}</h1>
          <div className="mt-2 flex flex-wrap items-center gap-x-2.5 gap-y-1.5 text-[12px] text-ink-600">
          ...
          <section className="mt-9 flex flex-wrap items-center gap-2.5">
            ...
            >
              ...
              {pending ? '保存中…' : skill.favourite ? '已收藏' : '收藏'}
            </button>
            <button
              type="button"
              data-testid="skill-add-to-session"
              onClick={() => setLoadGateOpen(true)}
              className="flex items-center gap-1.5 rounded-xl bg-ink-900 px-4 py-2.5 text-[13px] font-medium text-white transition-opacity hover:opacity-85 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
            >
  ...
  return (
    <Dialog open={open} onClose={onClose} title="在项目里加载这个 Skill" testId="skill-session-gate">
      <div className="space-y-3 text-[13px] leading-relaxed text-ink-600">
        ...
        {copyState === 'failed' && (
          <p className="text-[12px] text-ink-600" role="alert">当前环境不允许自动复制，请手动选中上面这行。</p>
        )}
      </div>
      <div className="flex items-center justify-end gap-2 pt-5">
        <button
          ...
          onClick={() => void copy()}
          className="flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-[13px] font-medium text-ink-700 transition-colors hover:bg-ink-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
        >
